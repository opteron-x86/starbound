...
use starbound_game::contracts::{generate_contract, ContractContext};
...
fn navigate_to_location(gs: &mut GameState, target_id: Uuid, from_dist: f32) {
    ...
    {
        let sys = gs.journey.current_system;
        let loc = gs.journey.current_location;
        let messages = starbound_game::contracts::check_contract_progress(
            &mut gs.journey, sys, loc,
        );
...
fn offer_contracts(gs: &mut GameState, npc_idx: usize) {
    ...
    let ctx = ContractContext {
        npc,
        systems: &gs.galaxy.systems,
        connections: &gs.galaxy.connections,
        factions: &gs.galaxy.factions,
        galactic_days: gs.journey.time.galactic_days,
    };
    let contract = generate_contract(&ctx);
